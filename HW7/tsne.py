...
def tsne(X=np.array([]), no_dims=2, initial_dims=50, perplexity=30.0, mode="tsne"):
    """
...
def show_gif(type, image, perplexity):
    ...
    for index, img in enumerate(image):
        ...
        #plt.clf()
        plt.scatter(img[0],img[1],img[2],img[3])
        #plt.savefig(f"./{type}/image_{index}.png")
        camera.snap()
...
if __name__ == "__main__":
    ...
    for per in perplexity:
        path = os.path.join('./tsne',str(per))
        os.makedirs(path, exist_ok=True)
        path = os.path.join('./ssne',str(per))
        os.makedirs(path, exist_ok=True)
    ...
    for mode in modes:
        for per in perplexity:
            ...
            Y, P, Q = tsne(X, 2, 50, per, mode)
            show_image(mode, img, per)
            show_gif(mode, img, per)
            show_pairwise_similarities(P, Q, labels, mode, per)
